...
class coeff_abr(object):
    ...
    def loadCoeff(self, oname, DayOrNight):
        '''
        ...
        if fname in flst:
            ...
            self.Coeff_M = self.__loadCoeffTxt_M(fname)
        else:
            Log.error('No Monthly Coeff TXT %s' % fname)
            self.error = True

        self.Coeff_D = self.__loadCoeffTxt_D(flst)

    def __loadCoeffTxt_M(self, fname):
        '''
    ...
    def __loadCoeffTxt_D(self, fnameLst):
        '''
        read txt, return ndarray
        '''
        retAry = None
...
def plot_omb(date_D, a_D, b_D,
    ...
    xx = np.array(xx)
    plt.pcolormesh(xx, yy , zz, cmap='jet', norm=norm, shading='gouraud', zorder=0)
